...
export const getPostsByCategory = (categoryId) => {
  return fetch(`http://localhost:8088/posts?category=${categoryId}`).then(
    (res) => res.json()
  );
};
...
export const getPostsByTitle = (title) => {
  return fetch(`http://localhost:8088/posts?title=${title}`).then((res) =>
    res.json()
  );
};

export const getPostsByTag = (tagId) => {
  return fetch(`http://localhost:8088/posts?tag=${tagId}`).then((res) =>
    res.json()
  );
};
